...
def _gold_exists(path: Path | None) -> bool:
...
def main() -> int:
    ...
    if args.validate_only:
        ...
        return 0
    if not args.execute:
        raise RuntimeError("use --validate-only, or explicitly pass --execute after human gold exists")
    ...
    if not _gold_exists(args.gold):
        raise RuntimeError("a non-empty human Label Studio gold export is required before Gemini execution")
    if incomplete:
        raise RuntimeError(
            "structured physical blocks are missing for selected pages: " + ", ".join(incomplete)
        )
