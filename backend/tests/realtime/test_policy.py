...
def test_all_compatibility_modes_use_one_adaptive_conversation_policy(
    ...
    assert "least demanding intervention" in prompts[0]
    assert "Do not ask substantially the same guiding question twice" in prompts[0]
    assert "Level 2 MUST explain exactly one missing concept" in prompts[0]
    assert "Level 3 MAY name two or three concepts" in prompts[0]
    assert "student's own wording" in prompts[0]
    assert "Do not require all four levels" in prompts[0]
    assert "A direct concept request may receive relevant facts" in prompts[0]
    assert "Do not combine levels in one turn" in prompts[0]
    assert "MUST leave out at least one substantive idea" in prompts[0]
    assert "MAY state one brief capability boundary once per active question" in prompts[0]
    assert "MUST immediately continue with one allowed progressive-help level" in prompts[0]
    assert "Do not repeat the boundary" in prompts[0]
    assert "another answer demand without student answer content MUST use Level 3" in prompts[0]
    assert "as terse labels or fragments only" in prompts[0]
    assert "MUST NOT explain another concept or connect the listed concepts" in prompts[0]
    assert "never add the remaining connection, outcome, or evidence" in prompts[0]
