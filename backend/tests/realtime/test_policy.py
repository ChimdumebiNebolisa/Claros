...
def test_all_compatibility_modes_use_one_adaptive_conversation_policy(
    ...
    assert "explain one missing concept" in prompts[0]
    assert "identify the small set of concepts" in prompts[0]
    assert "student's own wording" in prompts[0]
    assert "Do not require all four levels" in prompts[0]
    assert "Concept explanations may state relevant facts" in prompts[0]
    assert "must not compose the finished worksheet response" in prompts[0]
